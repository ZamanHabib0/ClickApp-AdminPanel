...
export default function MultiFileUpload({ error, showList = false, files, type, setFieldValue, sx, onUpload }) {
  const { getRootProps, getInputProps, isDragActive, isDragReject, fileRejections } = useDropzone({
    ...
    onDrop: (acceptedFiles) => {
      if (files) {
        setFieldValue('menu', [...files, ...acceptedFiles.map((file) => Object.assign(file, { preview: URL.createObjectURL(file) }))]);
      } else {
        setFieldValue(
          'menu',
  ...
  const onRemoveAll = () => {
    setFieldValue('menu', null);
  };

  const onRemove = (file) => {
    const filteredItems = files && files.filter((_file) => _file !== file);
    setFieldValue('menu', filteredItems);
  };
